...
if __name__ == '__main__':
    ...
    print(1)
    bondList = ['USA Notes 2.625% 31mar2025', 'USA Notes 1.25% 31dec2026', 'USA Notes 0.625% 31dec2027',
             'USA Bonds 3.25% 15may2042', 'USA Bonds 3% 15aug2052', 'Freddie Mac 0.375% 23sep2025',
             'New South Wales Treasury Corp 3% 20may2027',
             'Western Australian Treasury Corporation (WATC) 3.25% 20jul2028',
             'New South Wales Treasury Corp 2.25% 7may2041', 'Belgium OLO 3.75% 22jun2045', 'Japan JGB 0.4% 20sep2025',
             'Japan JGB 0.1% 20sep2027', 'Japan JGB 0.1% 20jun2031', 'Japan JGB 1% 20dec2035',
             'Japan JGB 1.7% 20sep2044',
    ...
    w = pd.read_csv('./data/target_weights.csv', header=None).iloc[:, 0].tolist()
    dates = get_trading_dates('2022-09-23', '2022-12-31')
    weightDf = pd.DataFrame(index=dates, columns=bondList, data=np.tile(w, (len(dates), 1)))
    test = backtest(0, './')
    ...
    # test.fastBacktest(weightDf)
    temp = pd.read_csv('extreme.csv')
    for lam, w in zip(temp['lambda'], temp['weights']):
        if lam not in [0, 1, 5, 15, 20]:
            continue
        os.chdir('D:/PycharmProjects/pythonProject2/robust_bond_portfolio-main/')
        path = './lambda' + str(lam)
        os.mkdir(path)
        os.chdir(path)
        values_list = w.strip('[]').split()
        w1 = [float(val) for val in values_list]
        dates = get_trading_dates('2022-09-23', '2022-12-31')
        weightDf = pd.DataFrame(index=dates, columns=bondList, data=np.tile(w1, (len(dates), 1)))
        test.fastBacktest(weightDf)
