...
def portfolio_construction(
    ...
    plot_turnover_distance(modest_linear, extreme_linear)
    plot_heatmaps(modest_linear, extreme_linear)
    modest_linear.to_csv('modest.csv')
    extreme_linear.to_csv('extreme.csv')
...
def run_portfolio_construction(
    ...
    for lambda_val in np.linspace(0.0, 20.0, 41):
        ...
        saddle_problem.solve(solver=cp.MOSEK, verbose=True)
